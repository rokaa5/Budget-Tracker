...
function SideTabIcon({
  ...
  return (
    <Pressable
      onPress={() => { pop(); onPress?.(); }}
      onPressOut={reset}
      style={{ alignItems: 'center', justifyContent: 'center', flex: 1, gap: 4, paddingBottom: 8 }}
    >
      <Animated.View style={{ transform: [{ scale }] }}>
        {children}
      </Animated.View>
      <Text style={{ fontSize: 10, color: selected ? DARK.active : DARK.inactive }}>
        {label}
      </Text>
    </Pressable>
  );
...
export function TabScreenWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Animated.View style={{ flex: 1, opacity, transform: [{ translateY }], backgroundColor: DARK.bg }}>
      {children}
    </Animated.View>
  );
}
...
function CustomTabBar({ state, descriptors, navigation }: any) {
  return (
    <View style={{
      ...
      <View style={{ flexDirection: 'row', height: TAB_BAR_HEIGHT + 20, alignItems: 'flex-end' }}>
        {state.routes.map((route: any, index: number) => {
          ...
          if (isCenter) {
            return <CenterTabIcon key={route.key} onPress={onPress} selected={isFocused} />;
          }
          ...
          return (
            ...
            >
              {index === 0
                ? <AntDesign name="home" size={24} color={isFocused ? DARK.active : DARK.inactive} />
                : <MaterialCommunityIcons name="dots-horizontal" size={24} color={isFocused ? DARK.active : DARK.inactive} />
              }
            </SideTabIcon>
          );
        })}
